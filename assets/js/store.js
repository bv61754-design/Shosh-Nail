...
(function(){
  ...
  var MINE_KEY  = 'shosh-nail-mine';
  var ADMIN_KEY = 'shosh-admin';
  var MINE_MAX  = 60;
  ...
  Store.set = function(path, value){
    if (!parts(path).length) return value;
  ...
  Store.exportFile = function(filename){
    var name = filename || ('shosh-nail-backup-' + stamp() + '.json');
    var text, blob, url, a;
  ...
  Store.login = function(pass){
    var real = Store.get('settings.adminPass', 'shosh1234');
    var ok = String(pass === undefined || pass === null ? '' : pass) === String(real);
